...
class Mur:
    ...
    def Update(self):
        ...
        if (self.x <= 90 <= self.x+self.width or self.x <= 110 <= self.x+self.width) and (self.y <= Y-10 <= self.y+self.height or  self.y <= Y+10 <= self.y+self.height):
            reset = True
            return True
...
def MainLoop():
    ...
    if cooldown > 0:
        ...
        list_mur.append(Mur(450,0,50,randint(10,100)))
        list_mur.append(Mur(450,randint(200,290),50,200))
    ...
    for mur in list_mur:
        ...
        can.create_rectangle(mur.x,mur.y,mur.x+mur.width,mur.y+mur.height,fill="black")
