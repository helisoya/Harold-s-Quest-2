...
def MiniGame_EV():
    global list_vautour,diff,temps,counter,pos2,pos1
    fenetre=Tk()
    ...
    def MainLoop():
        ...
        canvas.itemconfigure("temps",text="temps="+str(temps)[:2])
        temps-=1
        ...
        if hero.hp==0 :
           canvas.delete("all")
           canvas.create_text(237,200,text="Perdu",fill="red",font=("Helvetica",50))
           return
        if temps==0:
           canvas.delete("all")
           canvas.create_text(237,200,text="Gagné",fill="green",font=("Helvetica",50))
           return
        fenetre.after(5,MainLoop)
