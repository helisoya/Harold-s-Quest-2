...
def MiniGame_QW():
    global Curr,diff,available,Questions,target
    ...
    def Input(event):
        global target,Questions
        awnser = str(event.widget.gettags('current')[0])
        if "t"+Questions[Curr]["good"] == awnser:
            NextQuestion()
        else:
            target = "RickeyLandWaldo_Attr_Echec"
            


    waldo = PhotoImage(master=root,file="Data/Autres/QW/waldo.png")

    canvas = Canvas(root, width=500, height=450, background='white')
    canvas.create_image(250,150,image=waldo)
    canvas.create_rectangle(100,300,240,350,fill="gray",tags="t1")
    canvas.create_rectangle(260,300,400,350,fill="gray",tags="t2")
    canvas.create_rectangle(185,360,325,410,fill="gray",tags="t3")


    canvas.create_rectangle(100,240,400,290,fill="gray")
    canvas.create_text(250,265,text="Question",tags="question")
    canvas.create_text(170,325,text="A",tags=("t1","txt1"))
    canvas.create_text(330,325,text="B",tags=("t2","txt2"))
    canvas.create_text(255,385,text="C",tags=("t3","txt3"))


    canvas.pack()

    canvas.tag_bind("t1","<Button-1>",Input)
    canvas.tag_bind("t2","<Button-1>",Input)
    canvas.tag_bind("t3","<Button-1>",Input)
    ...
    def NextQuestion():
        ...
        if len(available) > 4:
            ...
            target = "RickeyLandWaldo_Attr_3"


    def Check():
        global target
        if target!="":
            TableauSuivant(target)
            root.destroy()
            EnableWindow() 
        else:
            root.after(15,Check)
